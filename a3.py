...
from movies import movie_db #movies = the tab (movies.py) and movie_db is the data/list
...
def director_by_title(matches: List[str]) -> List[str]:
    ...
    results = []
    for movie in movie_db:
        #if title of movie = argument, director appended
...
def title_by_actor(matches: List[str]) -> List[str]:
    ...
    results = []
    for movie in movie_db:
    #for in: asically means if matches is in the actors list, print title blah blah blah
...
#str means we are going to SPLIT(the prompt) a STRING(str) and match it with the functions we made above
...
pa_list: List[Tuple[List[str], Callable[[List[str]], List[Any]]]] = [
    ...
    (str.split("% appeard in what movies"), title_by_actor),
    (str.split("in what movies did % appear"), title_by_actor),
    (["bye"], bye_action),
]


#list of strings that takes source, finds patterns(the ones above) and see if they match, if they do, return the result
